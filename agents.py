...
class GeminiInterface(ModelInterface):
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            # Set up generation config with defaults and user overrides
            generation_config = {
                "temperature": kwargs.get("temperature", 0.7),
                "top_p": kwargs.get("top_p", 0.95),
                "top_k": kwargs.get("top_k", 40),
...
class DynamicSuggestionAgent:
    ...
    def __init__(self, llama_interface: LlamaCppInterface, db_manager: DatabaseManager):
        """Initialize the suggestion agent.
        
        Args:
            llama_interface (LlamaCppInterface): The LlamaCpp model interface.
            db_manager (DatabaseManager): The database manager.
        """
        self.llama_model = llama_interface
        self.db_manager = db_manager
    ...
    def get_suggestions(self, partial_query: str) -> List[str]:
        ...
        try:
            ...
            response = self.llama_model.generate(prompt, max_tokens=500, temperature=0.7)
...
class QueryOrchestrator:
    ...
    All LLM-based outputs are generated using GroqInterface only.
    Includes support for query cancellation.
    """
    
    @debug_logger
    def __init__(self, db_manager: DatabaseManager, api_key: str = None, model_name: str = "llama3-8b-8192"):
        """Initialize the query orchestrator.
        
        Args:
            db_manager (DatabaseManager): The database manager.
            api_key (str, optional): The API key for Groq. Defaults to None (uses env var).
            model_name (str, optional): The model name for Groq. Defaults to 'llama3-8b-8192'.
        """
        logger.debug("Initializing QueryOrchestrator...")
        self.groq_model = GroqInterface(api_key=api_key, model_name=model_name)
        self.db_manager = db_manager
        
        # Initialize agents with the GroqInterface instance
        ...
        self.sql_generator = SQLGenerator(self.groq_model, db_manager)
